...
| F12 | **Private payout via Tempo Zone** (testnet only, flagged): worker withdraws settled funds straight into Zone A by encrypted deposit; stretch: zone-funded jobs via withdrawal callback | Public chain shows only Vault → ZonePortal + amount; recipient and memo encrypted; Moderato tx links; README states it is testnet-only. **Done Sept 17**: `docs/e2e-zone-moderato-2026-09-17.txt` |
...
- **Private Zones (F12, decided Sept 17; testnet-only, behind a flag):** first, private payout — `withdrawToZone(token, amount, zoneId, encryptedPayload)` moves a user's Available balance into Zone A through `ZonePortal.depositEncrypted`, so the public chain shows only Vault → ZonePortal and the amount while the recipient and the job memo are encrypted to the zone sequencer. Second, if there is slack after real testers: zone-funded jobs — the Vault implements `IWithdrawalReceiver.onWithdrawalReceived(senderTag, token, amount, callbackData)` so a verifiable Zone withdrawal with `callbackData = (jobId, payer)` funds a job while the sender appears only as a commitment. Zones cannot host contracts and have no explorer; the README labels the whole feature testnet.
  **Resolved Sept 17 (live on Moderato):** the Zone A portal (`0x7069…9B23`, zoneId 6, chain 4217000006) is an older build than viem 2.56's ABI: `depositEncrypted` takes four arguments (no `tempoRefundRecipient`), the event is `EncryptedDepositMade`, and its sequencer derives the AES key without the sender binding that viem added in late July 2026. A payload built with the current scheme is accepted on-chain but never credited (one $2 deposit lost that way). The Vault therefore stores `legacyZonePortal[portal]` next to the allow-list and calls the matching ABI, and `@vouch/shared` ships `encryptZoneRecipient({ legacy })` mirroring viem's routine for both generations (round-trip tests in `zone.test.ts`). Zone auth tokens must be signed for the zone chain id, not the L1. The credit lands in the zone in the same second as the L1 block. Refunds on legacy portals return to the Vault as surplus and are attributed back by intake.
...
- F11 Earn while locked: Vault change + tests, curated vault list, picker toggle, job page "Earning" line, Moderato run with the open pathUSD vault; ask Tempo to allowlist the Vault on a mainnet vault. Then F12 private payout on Zone A — shipped Sept 17, live on Moderato. (Order fixed Sept 17: Earn first, Zones second, zone-funded jobs only with slack.)
...
- Stretch if ahead: zone-funded jobs (Vault as withdrawal receiver); virtual address per job.
